function [stego, bit_count, bit_positions] = WOW(cover, payload, params)
% -------------------------------------------------------------------------
...
%% Embedding simulator
[stego, bit_count, bit_positions] = EmbeddingSimulator(cover, rhoP1, rhoM1, payload*numel(cover), true);
...
function [y, bit_count, bit_positions] = EmbeddingSimulator(x, rhoP1, rhoM1, m, fixEmbeddingChanges)
    
    bit_positions = zeros([512 512]);
    n = numel(x);   
    ...
    [size_m, size_n] = size(x);
    bit_count = 0;
    for m = 1:size_m
        for n = 1:size_n
            if((randChange(m,n) < pChangeP1(m,n)) || (randChange(m,n) >= pChangeP1(m,n) && randChange(m,n) < pChangeP1(m,n)+pChangeM1(m,n)))
